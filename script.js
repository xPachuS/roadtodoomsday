(function () {
  ...
  var seen = {}, lv = "all", fmt = "all", hide = false, nospoil = true, days = 0;
  ...
  function build() {
    ...
    ERAS.forEach(function (e, ei) {
      ...
      D.forEach(function (d, i) {
        ...
        html += '<li class="item" data-i="' + i + '">' +
          ...
          '<span class="meta">' +
          '<span class="pill ' + (d.level === 1 ? 'p1">Imprescindible' : 'p2">Recomendada') + '</span>' +
    ...
    var step = 360 / N, ticks = "";
    for (var i = 0; i < N; i++) {
      var a = (-90 + step / 2 + i * step) * Math.PI / 180;
      var r2 = 112, r1 = D[i].level === 1 ? 86 : 98;
      ...
      ticks += '<g class="tkg" data-i="' + i + '"><title>' + (i + 1) + '. ' + esc(D[i].title) + '</title>' +
  ...
  function collect() {
    rows = D.map(function (d, i) {
      ...
      return {
        ...
        tick: document.querySelector('.tkg[data-i="' + i + '"] .tk')
      };
    });
  ...
  function update() {
    ...
    rows.forEach(function (r) {
      ...
      eraTot[d.era]++;
      if (s) eraSeen[d.era]++;
      var ok = (lv === "all" || d.level === 1) && (fmt === "all" || d.kind === fmt) && !(hide && s);
      r.li.hidden = !ok;
  ...
  $("n-fmt-m").textContent = count(function (d) { return d.kind === "m"; });
  $("n-fmt-s").textContent = count(function (d) { return d.kind === "s"; });
  syncControls();
